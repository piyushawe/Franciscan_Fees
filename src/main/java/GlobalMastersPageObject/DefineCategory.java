...
public class DefineCategory {
	WebDriver dr;
	Utility u= new Utility();
	String pg= "DefineCategory";

	@FindBy(id="ContentPlaceHolder1_txtCategoryName_TextBox")WebElement category;
	...
	public void openDefineCategory()
	{
	   WebElement menu= dr.findElement(By.xpath("//img[@src='/Images/layout/Global-Masters.png']"));
	   Actions builder= new Actions(dr);
	   builder.moveToElement(menu).build().perform();
	   dr.findElement(By.linkText("Define Category")).click();
	   dr.switchTo().frame(dr.findElement(By.id("Define Category")));
	 ...
	 public void enterCategory(String lang)
	 {
		 category.sendKeys(lang);
	 }
	 ...
	 public void clickView(String school, Collection<String> sc)throws IOException
	 {
		...
		if (cells.size()>1) {
			for (WebElement cell : cells) {
				cell.findElement(By.xpath("//input[starts-with(@id,'ContentPlaceHolder1_rptCategory')]")).click();
				break;
			}
