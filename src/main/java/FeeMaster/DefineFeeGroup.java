...
public class DefineFeeGroup {
	WebDriver dr;
	Utility u= new Utility();
	String pg= "DefineFeeGroup";

	@FindBy(id="ContentPlaceHolder1_txtGroupName_TextBox")WebElement group;
	@FindBy(id="ContentPlaceHolder1_chkIsSpecial")WebElement isspecial;
	...
	public DefineFeeGroup(WebDriver d)
	{
	   this.dr=d;
	   PageFactory.initElements(d, this);
	}
	public void openDefineFeeGroup()
	{
	   WebElement menu= dr.findElement(By.xpath("//img[@src='/Images/layout/Fee-Master.png']"));
	   Actions builder= new Actions(dr);
	   builder.moveToElement(menu).build().perform();
	   dr.findElement(By.linkText("Define Fee Group")).click();
	   dr.switchTo().frame(dr.findElement(By.id("Define Fee Group")));
	...
	{
		group.sendKeys(gname);
	}
	public void selectIsSpecial()
	{
		isspecial.click();
	}
	...
	public void clickView(String school, Collection<String> sc)throws IOException
	{
		...
		if (cells.size()>1) {
			for (WebElement cell : cells) {
				cell.findElement(By.xpath("//input[starts-with(@id,'ContentPlaceHolder1_rptFeeGroup')]")).click();
				break;
			}
